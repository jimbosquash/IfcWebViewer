...
import { InfoPanelContext, InfoPanelDataProvider, InfoPanelProps } from "../../context/InfoPanelContext";
...
export const InfoPanel = () => {
   ...
   const [showToolTip, setShowToolTip] = useState<boolean>(false);
    ...
    useEffect(() => {
        if(infoPanelContext)
            setPanelData(infoPanelContext.data)
    },[infoPanelContext])
    ...
    return (
        <Box component={"div"} 
            style={infoBoxStyle}
                width="700px">
            {/* file name */}
            {infoPanelData && Object.entries(infoPanelData).map(([key,value],index) => (
                <div
                key={index}
                style={{
                    ...subBoxStyle,
                    ...(index !== Object.entries(infoPanelData).length - 1 ? subBoxNotLastChildStyle : {}),
                    ...(hoveredIndex === index ? subBoxHoveredStyle : {})
                }}
                onMouseEnter={() => setHoveredIndex(index)}
                onMouseLeave={() => setHoveredIndex(null)}
                onClick={index === Object.entries(infoPanelData).length - 1 ? () => setShowToolTip(!showToolTip) : undefined}
                >
          {value}
          {index === Object.entries(infoPanelData).length - 1 && showToolTip && (
