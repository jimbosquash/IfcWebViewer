...
export class HVACViewer extends OBC.Component {
    ...
    handelSelectedGroupChanged(data: SelectionGroup) {
        ...
        let managedTypeElements = buildingElements.filter(el => this._managedTypes.find(partialType => el.type.includes(partialType)))
        managedTypeElements = [...managedTypeElements, ...(buildingElements.filter(el => GetPropertyByName(el, knownProperties.ProductCode)?.value.includes("TE")))]

        if(!managedTypeElements) {
            if(this._foundElements.length !== 0)         
                this.onFoundElementsChanged.trigger([]); // this means that the group had hvac elements
